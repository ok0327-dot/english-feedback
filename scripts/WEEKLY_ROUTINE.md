...
- `docs/radio.html`의 `<pre id="radio-src">...</pre>` 안 **라디오 대본**을, **약 15~20분 분량(120~200줄)**의 생생한 영어 라디오쇼로 교체해라. 두 진행자(민지=따뜻한 호스트, 알렉스=영어 코치)가 **전부 영어로**(immersion) 대화한다.
  - **한 주를 통째로 망라**: 이번 주 AI 교정 + 강사 교정을 최대한 많이(읽고→자연스러운 버전→왜 그런지 + 미니 예문), 약점 문법 심화(수일치·관사 등), 발음 포인트, 어휘 정의(영어로), 그리고 **그 주 주제 관련 표현·단어(Topic Talk)**.
  - **Topic Talk**: 그 주 주제(투자/부동산, 회사/업무, 사무용품, 회의/기획, 그림 묘사 등)별로 **A2~B1 수준의 쉬운 일상 표현·단어 3~5개**를 예문과 함께 가르쳐라. 교정 리뷰뿐 아니라 '주제로 쓸 수 있는 새 표현'을 더해 학습량을 키운다.
  - **난이도 A2~B1**: 설명·진행은 쉬운 단어와 짧고 명확한 문장으로.
  - 형식 엄수: **한 줄에 `진행자|lang|문장`** (진행자=민지 또는 알렉스, lang=`en`, 문장 안에 `|` 금지, 빈 줄 금지). **형식이 깨지면 음성 재생·MP3 생성이 안 된다.**
