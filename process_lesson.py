...
2. **화자 분리**: 각 발화 앞에 화자를 표시
   - [Tutor]: 원어민 튜터의 발화 (자연스러운 영어, 교정/설명)
   - [Student]: 한국인 학습자의 발화 (문법 오류가 있을 수 있음)
...
def generate_feedback(transcript):
    ...
    prompt = f"""Role: 당신은 10년 경력의 영어 회화 교정 전문 튜터이자 언어학 박사입니다.
Task: 한국인 학습자의 전화영어 수업을 분석하여, 실용적이고 풍부한 학습 자료를 제공합니다.

아래는 화자 분리가 완료된 전화영어 수업 전사 내용입니다.
[Tutor]는 원어민 튜터, [Student]는 한국인 학습자입니다.
...
분석 지침:
- [Student]의 발화만 교정 대상입니다
- [Tutor]가 수업 중 교정해준 내용이 있다면 반드시 반영하세요
- 단순 문법 오류뿐 아니라 어색한 표현, 직역체도 잡아주세요
- 각 섹션을 풍부하고 구체적으로 작성해주세요

아래 형식으로 피드백을 작성해주세요.
한국어로 설명하되, 영어 표현과 예문은 영어 원문을 유지하세요.

---

## 📊 오늘의 수업 요약
- 수업 주제와 핵심 토픽을 2~3줄로 정리

## 🔧 문법 및 표현 교정

각 교정 항목을 아래 형태로 작성 (최소 5개 이상):
- ❌ 학습자 원문: "실제 사용한 문장"
- ✅ 교정: "자연스러운 문장"
- 💡 설명: 왜 틀렸는지, 어떤 문법/표현 규칙인지

## 📗 핵심 단어장

수업에서 나온 중요 단어/표현을 아래 형식으로 정리 (최소 8개):
...
- 동의어: 비슷한 표현 1~2개
- 예문: 수업 맥락을 활용한 실용 예문
- 💡 팁: 한국인이 자주 혼동하는 포인트나 활용 팁

※ 수업에서 직접 사용된 단어 + 관련 확장 어휘를 모두 포함

## 📘 문법 포인트

수업에서 드러난 문법 이슈를 상세히 분석 (최소 3개):

각 포인트마다:
- **문법 규칙 이름** (예: 현재완료 vs 과거시제)
- 📌 규칙 설명: 언제, 어떻게 쓰는지 명확하게
- ❌ 학습자 오류: 실제 틀린 문장
- ✅ 올바른 표현: 교정된 문장
- 📝 추가 예문 2~3개 (다양한 상황에서의 활용)
- ⚠️ 한국인 주의점: 한국어와 영어의 구조적 차이로 인해 자주 틀리는 이유

## 🗣️ 실전 회화 패턴

수업 내용과 관련된 실전 회화 패턴 (최소 5개):

각 패턴마다:
- **패턴**: 핵심 구문 틀 (예: "I've been ~ing for ...")
- 의미: 언제 사용하는 패턴인지
- 상황 1: 일상 대화에서의 활용 예문
- 상황 2: 직장/비즈니스에서의 활용 예문
- 상황 3: 다른 상황에서의 활용 예문
- 🔄 변형: 비슷하지만 다른 뉘앙스의 패턴

## 🔄 반복 실수 패턴 분석
- 이 수업에서 2회 이상 반복된 문법 실수나 표현 패턴을 추출
- 각 패턴에 대해 핵심 규칙 + 교정 전/후 비교

## ✅ 잘한 점
- 학습자가 효과적으로 사용한 표현이나 커뮤니케이션 전략

## 🎯 개선 포인트 TOP 3
1. 가장 시급한 개선 사항 + 구체적 연습 방법
2. 두 번째 + 구체적 연습 방법
3. 세 번째 + 구체적 연습 방법

## 📝 복습용 영작 연습
- 오늘 교정된 표현들을 활용한 연습 문제 7개
- 형식: 한국어 문장 → (학습자가 영작) → 모범 답안
- 쉬운 문제부터 어려운 문제 순서로 배치
- 예:
  - Q: "나는 그 프로젝트에 3년 동안 참여해왔다."
  - A: "I have been involved in the project for three years."
...
총평: (2~3줄로 핵심 피드백 요약 + 다음 수업에서 집중할 포인트)
